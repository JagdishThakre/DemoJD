(function(){
    ...
    '<div class="flex-calendar">'+
      ...
      '<div class="days" ng-repeat="week in weeks">'+
        '<div class="day"'+
          ...
          'ng-class="[getDayClass(day), {beforeDay:isBeforeDate(day), selected: isDefaultDate(day), disabled: day.disabled, out: !day}]"'+
          'ng-click="onClick(day, $index, $event)"'+
        '>'+
          '<div class="number">{{day.day}}</div>'+
        '</div>'+
      '</div>'+
       '<div class="row" style="color: black;font-size:11px;background-color:#fff"><div ng-repeat="text in textStatus" class="col-25"><div><div class="{{text.className}}"></div></div><div style="text-align: center;">{{text.txt}}</div></div>'+
    '</div>';
  ...
  Controller.$inject = ['$scope' , '$filter','reservationDataService'];
  function Controller($scope , $filter,reservationDataService) {
   
     console.log(reservationDataService);
     $scope.holidayEveryWeek = [];
     $scope.weekNumberHoliday = [];
     $scope.bookingList= reservationDataService.bookingTimes;
     angular.forEach($scope.bookingList,function(item){
        if(item.is_holiday == true){
          $scope.holidayEveryWeek.push(item.day);
        }
     })
     angular.forEach($scope.holidayEveryWeek,function(item){
         switch(item){
            case 'Monday':
              $scope.weekNumberHoliday.push(0);
              break;
              case 'Tuesday':
              $scope.weekNumberHoliday.push(1);
               break;
            case 'Wednesday':
              $scope.weekNumberHoliday.push(2);
               break;
              case 'Thursday':
              $scope.weekNumberHoliday.push(3);
               break;
              case 'Friday':
              $scope.weekNumberHoliday.push(4);
               break;
              case 'Saturday':
              $scope.weekNumberHoliday.push(5);
               break;
              case 'Sunday':
              $scope.weekNumberHoliday.push(6);
               break;
             default:
              $scope.weekNumberHoliday = []; 
               break;
    ...
    $scope.bookedDates = reservationDataService.bookedDates || [];
    $scope.days = [];
    ...
    $scope.isBeforeDate = isBeforeDate;
    ...
    $scope.textStatus =[{className:'calColorCl',txt:'Closed'},{className:'calColorPA',txt:'Partially available'},
    {className:'calColorCA',txt:'Current date'},{className:'calColorNA',txt:'Not available'},
    ];
    ...
    function createMappedEvents(){
     ...
     // console.log( $scope.mappedEvents)
    }
    ...
    function calculateWeeks() {
      ...
      for (var day = 1; day < daysInCurrentMonth + 1; day += 1) {
        ...
        week[dayNumber] = {
          ...
          day: day,
          timings:$scope.bookingList[dayNumber],
          date: date,
          _month : date.getMonth() + 1
        };
         angular.forEach($scope.weekNumberHoliday,function(item){
        if(dayNumber == item){
          var calcWeekHoilday = { foo: 'holiday',
          date: date,
          eventClass: 'holiday' };
          $scope.events.push(calcWeekHoilday);
