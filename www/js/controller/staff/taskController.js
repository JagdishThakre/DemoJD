app.controller("TaskCtrlStaff", function(
  $scope,
  $rootScope,
  $cordovaKeyboard,
  unitIdsList,
  ...
  /*************task list refresh *************/
  $scope.refreshMyTaskList = function() {
    ...
    if ($rootScope.authoriedUser == true) {
      ...
      var taskData = {
        ...
        priority: parseInt(localStorage.getItem("taskPriority")),
      ...
      if ($scope.isOnline() == true) {
        getNotificationstaff.taskListstaff().save(
          taskData,
          function(res) {
            ...
            if (res.results.length > 0) {
              ...
              setTimeout(function() {
                if ($scope.searchThisUnit == "") {
                  ...
                  $scope.messagesJson.defaultCount = $scope.msgLength;
                  $scope.page_number = parseInt($scope.page_number) + 1;
                  $scope.messagesJson.currentId = $scope.unitNumberKeyWord;
                  $scope.messagesJson.pageNo = $scope.page_number;
                  $scope.messagesJson.emptyIt = false;
                }
              });
  ...
  $scope.getTaskLisstaff = function() {
    if ($rootScope.task_status == undefined) {
      $rootScope.task_status = "";
    } else if ($rootScope.task_status) {
      $rootScope.task_status = $rootScope.task_status;
  ...
  if ($scope.loaded == 1) {
    ...
    if ($scope.messagesJson.emptyIt == true) {
      ...
      setTimeout(function() {
        $scope.getTaskLisstaff();
      });
    } else {
      if (
        $scope.messagesJson.currentId != undefined &&
        $scope.messagesJson.currentId != ""
      ) {
        ...
        if ($scope.messagesJson.currentId.length != 0) {
          $scope.show = true;
        }
        $scope.search1 = {
          unit_number: $scope.messagesJson.currentId
        };
      } else {
        $scope.search1 = {
          unit_number: ""
  ...
  // $scope.getMyTskTabHighCount = function() {
  ...
  // $scope.getMyTskTabMediumCount = function() {
  ...
  // $scope.getMyTskTabNormalCount = function() {
  ...
  $scope.taskDetails = function(value) {
    ...
    localStorage.setItem("taskDetails", value._id);
  ...
  $scope.getTaskDetail = function() {
    if ($rootScope.authoriedUser == true) {
      $ionicLoading.show();
      var taskDetailsData = {
        tasks_id: localStorage.getItem("taskDetails"),
        token: $localStorage.User.token,
        property_id: $localStorage.User.property_id,
      ...
      getNotificationstaff.getTaskDetails().save(
        ...
        function(res) {
          ...
          if (res.details.task_complete_date) {
            $scope.completeDate = moment(res.details.task_complete_date).format(
              "ddd, MMM Do, YYYY [at] hh:mm A"
            );
          }
  ...
  /**** For Delete My-Task Start ****/
  $scope.deleteMyTask = function(taskId, estimateAccptStatus) {
    if (estimateAccptStatus == 1) {
      $scope.alertPop(
        "Alert",
        "Cant't delete Task,Because its estimate Approved !"
      );
    } else {
  ...
  $scope.getMyTaskDetail = function(editTaskDetails) {
    globalShareService.globalShareVars = {
      editMyTaskData: editTaskDetails
    };
    $state.go("editMyTask");
  };
  /*Edit My Task Ends*/
  ...
  $scope.editTask = function(editedTaskData, datetimeValue, taskType) {
    var editTaskFinishB4Time = new Date(datetimeValue).toISOString();
    $scope.assignByName =
    ...
    var editPostData = {
      ...
      task_type: editedTaskData.taskType,
    ...
    $ionicLoading.show();
    getNotificationstaff.editStaffsMyTask().save(
      editPostData,
      function(res) {
        $ionicLoading.hide();
        if (res.code == 200) {
          $scope.alertPop("Alert", res.message, "taskDetails");
  ...
  $scope.editEntry = function(taskDetails) {
  ...
  $scope.updateEntryMyTask = function(updateEntry) {
    var workTask = {};
    $scope.oldImg = [];
    ...
    localStorage.setItem("taskDetails", localStorage.getItem("taskId"));
  ...
  $scope.editEsitmate = function(taskDetails) {
    localStorage.setItem("taskServiceId", taskDetails.service_id._id);
    localStorage.setItem("taskId", taskDetails._id);
  ...
  $scope.updateEstimation = function(updateEstimation) {
    $scope.oldImg = [];
  ...
  /**** For Closed My-Task Start ****/
  $scope.closeMyTask = function(closeTaskId) {
    $scope.closeWoPop = $ionicPopup.confirm({
      title: "Confirm",
      cssClass: "WoMngrPopHead",
  ...
  /*******************task detaisl***********/
  $scope.goTaskDetails = function(id) {
    localStorage.setItem("service_id", id);
    $state.go("woDetailspgStaff");
  };
